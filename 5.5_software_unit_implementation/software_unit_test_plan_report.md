...
#### 7.3.16 残 2 ユニット骨格(Step 19 B の TDD Red で詳細化)
...
**合計ケース数目安(全 17 ユニット):** **≥ 145 件**(代表 5 + UNIT-003.1 + UNIT-003.2 + UNIT-001.5 + UNIT-001.2 + UNIT-002.1 + UNIT-002.2 + UNIT-001.3 + UNIT-002.3 + UNIT-004.2 + UNIT-005.1 = 259 件実測 + 骨格 2 = 14 件の目安)。最終的な件数は Step 19 B の TDD で増減する見込み。
...
| UNIT-005.1 | **21**(うち UT-005.1-01..20 展開後 21、内訳:正常系 8 + 異常系 6 + RCM 4 + 不変性 1 + 契約 1 + 並行/網羅性 1)| **21** | 0 | 0 | **100.00% / 100.00% / 100%(MC/DC 試験設計担保、骨格 90% から 100% に引き上げ、`start` 3 分岐 + `confirm_resume` 4 分岐 + try/except 例外耐性 3 経路 + `_safe_enqueue` 例外 1 経路 + Accepted/Rejected 分岐 全網羅、API 委譲層、`Settings.model_dump()` で pydantic を `Mapping[str, object]` に変換、ValidationApi Protocol で SEP-001 分離維持)** | 2026-04-30 | Step 19 B16 PR マージコミット(TBD)|
| UNIT-005.2 | ≥ 6 | — | — | — | — | — | — |
| UNIT-005.3 | ≥ 8 | — | — | — | — | — | — |
| **合計** | **≥ 145** | — | — | — | **— / — / —** | — | — |
