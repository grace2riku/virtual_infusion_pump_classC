...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.8 | ドラフト(セルフ承認、Step 19 B1 反映:SOUP-001〜011 を正式登録に昇格、TOOL-008〜012 / CFG-009 / CFG-011 を「予定」から運用中に遷移、TD-001 / TD-001a 新規登録、CFG-010 は計画のまま据置、自己参照 v0.8) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.10 | 継続更新(ステップ毎に追記、Step 19 B1 反映:Python パッケージ骨格 + `pyproject.toml` + `unit-test.yml` の CI 整備)|
...
| (予定) CI-TD-001b | ユニット試験ケース(UNIT 別)| `tests/unit/test_*.py` | Step 19 B2 以降で UNIT-001.1 から順次 TDD Red で追加 |
