...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.14 | ドラフト(セルフ承認、Step 19 B14:UNIT-002.3 実施結果を §9.2 / §11 に反映、Pass 12 tests / カバレッジ 100%(stmt/branch)/ MC/DC 「—」据置(Inc.1 no-op スタブ)。§7.3.13 を新規詳細化(骨格の「≥ 3 / —」を 12 ケース / stmt-branch 100% に展開、運用性 4 + 専門性 5 論点を推奨方針で解消、UT 申し送りなし)、UNIT-001.3 行のコミット欄を Step 19 B13 PR #21 マージ SHA `6c1a374` で確定、MINOR 区分・CR 不要、残骨格を 5 → 4 ユニットに更新)|
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.21 | ドラフト(セルフ承認、Step 19 B14 反映:CI-SRC-005 に UNIT-002.3 Event Injection Stub v0.1 追記(仮想ハードウェア層 UNIT-002.* 全 4 ユニット完成)、CI-TD-001o 新規登録、CI-TD-001c 対象から UNIT-002.3 を除外、CI-DOC-UTPR v0.14 反映、CI-DOC-DEVSTEPS v0.23 反映、自己参照 v0.21) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.23 | 継続更新(ステップ毎に追記、Step 19 B14 反映:UNIT-002.3 Event Injection Stub TDD 実装、カバレッジ 100%(stmt/branch)達成 / MC/DC 「—」据置(Inc.1 no-op スタブ)、SRS-032 + SRS-I-040(Inc.2 placeholder)実装で **仮想ハードウェア層 UNIT-002.* 全 4 ユニット完成**、残 5 → 4 ユニット、UTPR §7.3.13 を新規詳細化)|
...
| (予定) CI-TD-001c | UNIT-004.2 / 005.x 試験 | `tests/unit/test_*.py` | Step 19 B15+ 以降で UNIT-004.2 Resume Confirmation Gate などを順次 TDD Red で追加 |
