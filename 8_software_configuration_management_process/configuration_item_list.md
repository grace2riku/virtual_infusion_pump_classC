...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.4 | ドラフト(セルフ承認、Step 19 B4:UNIT-002.4 実施結果を §9.2 / §11 に反映、Pass 18 tests / カバレッジ 100% / MC/DC 100%。§7.3.3 を SDD §4.3 に整合化(Logger 据置 / DI 採用 / 逆転安全側)、MINOR 区分・CR 不要)|
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.11 | ドラフト(セルフ承認、Step 19 B4 反映:CI-SRC-005 に UNIT-002.4 v0.1 を追記、CI-TD-001e 新規登録、CI-DOC-UTPR v0.4 反映、自己参照 v0.11) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.13 | 継続更新(ステップ毎に追記、Step 19 B4 反映:UNIT-002.4 HW-side Failsafe Timer TDD 実装、カバレッジ 100% / MC/DC 100% 達成、UTPR §7.3.3 を SDD §4.3 に整合化、Logger 据置 + DI 採用 + 逆転安全側の 3 設計判断)|
...
| (予定) CI-TD-001c | UNIT-001.2 / 001.3 / 001.5 / 002.1 / 002.2 / 002.3 / 003.x / 004.x / 005.x 試験 | `tests/unit/test_*.py` | Step 19 B5+ 以降で UNIT-003.3 などを順次 TDD Red で追加 |
