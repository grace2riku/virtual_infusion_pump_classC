...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | 流量制御コア(UNIT-001.*)| `src/vip_ctrl/` | **UNIT-001.1 State Machine v0.1**(Step 19 B2、`state_machine.py`、148 stmt、カバレッジ 100%、MC/DC 100%、RCM-019 実装)+ **UNIT-001.2 Control Loop v0.1**(Step 19 B10、`control_loop.py`、94 stmt、カバレッジ 100%、MC/DC 100%、RCM-004 SW 送出側実装:100 ms `PERIOD_SEC` + heartbeat 順序保証 + dose-based AUTO_STOP + `tick()` テストフック + Pump/Observer Protocol 同居)+ **UNIT-001.4 Flow Command Validator v0.1**(Step 19 B3、`flow_validator.py`、56 stmt、カバレッジ 100%、MC/DC 100%、RCM-001 実装)+ **UNIT-001.5 SW Watchdog v0.1**(Step 19 B9、`watchdog.py`、78 stmt、カバレッジ 100%、MC/DC 100%、RCM-003 SW 側実装:300 ms `HEARTBEAT_TIMEOUT` + `check_once` テストフック + クロック DI + 階層防御 SW<HW 時間順序確認)+ UNIT-001.3 骨格のみ(`__init__.py` のみ、Step 19 B11+ で TDD)| C | 運用中(UNIT-001.1 / 001.2 / 001.4 / 001.5 完了、UNIT-001.3 は TDD 未着手) |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.12 | ドラフト(セルフ承認、Step 19 B12:UNIT-002.2 実施結果を §9.2 / §11 に反映、Pass 10 tests / カバレッジ 100% / MC/DC 100%。§7.3.11 を新規詳細化(骨格の「≥ 6 / —」を 10 ケース / 100% に展開、運用性 2 + 専門性 5 論点を推奨方針で解消)、MINOR 区分・CR 不要、残骨格を 7 → 6 ユニットに更新)|
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.19 | ドラフト(セルフ承認、Step 19 B12 反映:CI-SRC-005 に UNIT-002.2 Pump Observer v0.1 追記、CI-TD-001m 新規登録、CI-DOC-UTPR v0.12 反映、CI-DOC-DEVSTEPS v0.21 反映、自己参照 v0.19) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.21 | 継続更新(ステップ毎に追記、Step 19 B12 反映:UNIT-002.2 Pump Observer TDD 実装、カバレッジ 100% / MC/DC 100% 達成、SRS-031/I-020 観測契約完成で Control Loop の PumpSnapshotObserver Protocol が実体化、残 7 → 6 ユニット、UTPR §7.3.11 を新規詳細化)|
...
| (予定) CI-TD-001c | UNIT-001.3 / 002.3 / 004.2 / 005.x 試験 | `tests/unit/test_*.py` | Step 19 B13+ 以降で UNIT-001.3 Command Handler などを順次 TDD Red で追加 |
