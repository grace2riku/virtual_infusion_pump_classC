...
| CI-SRC-005 | 仮想ハードウェアモデル(UNIT-002.*)| `src/vip_sim/` | **UNIT-002.1 Pump Simulator v0.1**(Step 19 B11、`pump_simulator.py`、89 stmt、カバレッジ 100%、MC/DC 100%、SRS-030/031/P01 実装:一次遅れ τ=0.5 s + Decimal 精度 + RLock 並行 + 5 getter で SRS-031 観測契約 + RCM-004 HW 側被呼出 `force_stop_failsafe` 冪等 + 9999.9 mL 上限 overflow 警告)+ **UNIT-002.4 HW-side Failsafe Timer v0.1**(Step 19 B4、`failsafe_timer.py`、78 stmt、カバレッジ 100%、MC/DC 100%、RCM-004 HW 側実装)+ UNIT-002.2〜002.3 骨格のみ(`__init__.py`、Step 19 B1)| C | 運用中(UNIT-002.1 / 002.4 完了、UNIT-002.2 / 002.3 は TDD 未着手) |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.11 | ドラフト(セルフ承認、Step 19 B11:UNIT-002.1 実施結果を §9.2 / §11 に反映、Pass 21 tests / カバレッジ 100% / MC/DC 100%。§7.3.10 を新規詳細化(骨格の「≥ 10 / 95%」を 21 ケース / 100% に展開、運用性 3 + 専門性 5 論点を推奨方針で解消)、MINOR 区分・CR 不要、残骨格を 8 → 7 ユニットに更新)|
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.18 | ドラフト(セルフ承認、Step 19 B11 反映:CI-SRC-005 に UNIT-002.1 Pump Simulator v0.1 追記、CI-TD-001l 新規登録、CI-DOC-UTPR v0.11 反映、CI-DOC-DEVSTEPS v0.20 反映、自己参照 v0.18) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.20 | 継続更新(ステップ毎に追記、Step 19 B11 反映:UNIT-002.1 Pump Simulator TDD 実装、カバレッジ 100% / MC/DC 100% 達成、SRS-030/031/P01 + RCM-004 HW 側被呼出実装で制御系コア単体動作完結、残 8 → 7 ユニット、UTPR §7.3.10 を新規詳細化)|
...
| (予定) CI-TD-001c | UNIT-001.3 / 002.2 / 002.3 / 004.2 / 005.x 試験 | `tests/unit/test_*.py` | Step 19 B12+ 以降で UNIT-002.2 Pump Observer などを順次 TDD Red で追加 |
