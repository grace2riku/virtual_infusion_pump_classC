...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | 流量制御コア(UNIT-001.*)| `src/vip_ctrl/` | **UNIT-001.1 State Machine v0.1**(Step 19 B2、`state_machine.py`、148 stmt、カバレッジ 100%、MC/DC 100%、RCM-019 実装)+ UNIT-001.2〜001.5 骨格のみ(`__init__.py` のみ、Step 19 B2+ で TDD)| C | 運用中(UNIT-001.1 完了、UNIT-001.2〜001.5 は TDD 未着手) |
...
| CI-SRC-005 | 仮想ハードウェアモデル(UNIT-002.*)| `src/vip_sim/` | 骨格のみ(`__init__.py`、Step 19 B1)| C | 骨格完了、UNIT-002.* 実装は TDD 未着手 |
| CI-SRC-006 | 永続化(UNIT-003.*)| `src/vip_persist/` | 骨格のみ(Step 19 B1)| C | 骨格完了、TDD 未着手 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.2 | ドラフト(セルフ承認、Step 19 B2:UNIT-001.1 実施結果を §9.2 / §11 に反映、Pass 62 tests / カバレッジ 100% / MC/DC 100%)|
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.9 | ドラフト(セルフ承認、Step 19 B2 反映:CI-SRC-001 UNIT-001.1 v0.1 運用中化、CI-SRC-005/006/007/008/009 骨格追加、CI-TD-001b 新規登録、CI-DOC-UTPR v0.2 反映、自己参照 v0.9) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.11 | 継続更新(ステップ毎に追記、Step 19 B2 反映:UNIT-001.1 State Machine TDD 実装、カバレッジ 100% / MC/DC 100% 達成)|
...
| (予定) CI-TD-001c | UNIT-001.2〜005.3 試験 | `tests/unit/test_*.py` | Step 19 B2+ 以降で UNIT-001.2 から順次 TDD Red で追加 |
