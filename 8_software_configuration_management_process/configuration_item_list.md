...
| CI-SRC-006 | 永続化(UNIT-003.*)| `src/vip_persist/` | **UNIT-003.3 Atomic File Writer v0.1**(Step 19 B5、`atomic_writer.py`、78 stmt、カバレッジ 100%、MC/DC 100%、RCM-015 前提実装:temp→rename + 1 世代 bak)+ **共通データモデル v0.1**(Step 19 B6、`records.py`、37 stmt、カバレッジ 100%、pydantic frozen モデル `Settings` / `RuntimeState` / `RawPersistedRecord` / `TrustedRecord`、UNIT-003.1 Serializer + UNIT-004.1 Integrity Validator で共有)+ UNIT-003.1 / 003.2 骨格のみ(`__init__.py`、Step 19 B1)| C | 運用中(UNIT-003.3 + 共通データモデル完了、UNIT-003.1 / 003.2 は TDD 未着手) |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.6 | ドラフト(セルフ承認、Step 19 B6:UNIT-004.1 実施結果を §9.2 / §11 に反映、Pass 33 tests / カバレッジ 100% / MC/DC 100%。§7.3.5 を SDD §4.5 に整合化(戻り値型 FailsafeRecommended 統一 / pydantic 管轄ケース差し替え / FutureTimestamp 退け AccumulationExceedsDose 採用 / ERROR 不変条件差し替え StateContradiction 採用)、MINOR 区分・CR 不要、代表 5 ユニット全完了)|
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.13 | ドラフト(セルフ承認、Step 19 B6 反映:CI-SRC-007 に UNIT-004.1 v0.1 昇格、CI-SRC-006 に共通データモデル v0.1 追記、CI-TD-001g 新規登録、CI-DOC-UTPR v0.6 反映、自己参照 v0.13) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.15 | 継続更新(ステップ毎に追記、Step 19 B6 反映:UNIT-004.1 Integrity Validator TDD 実装、カバレッジ 100% / MC/DC 100% 達成、代表 5 ユニット全完了、UTPR §7.3.5 を SDD §4.5 に整合化(4 件不整合解消:戻り値型統一 / pydantic 管轄差し替え / FutureTimestamp 退け / ERROR 不変条件差し替え))|
...
| (予定) CI-TD-001c | UNIT-001.2 / 001.3 / 001.5 / 002.1 / 002.2 / 002.3 / 003.1 / 003.2 / 004.2 / 005.x 試験 | `tests/unit/test_*.py` | Step 19 B7+ 以降で UNIT-003.1 Serializer などを順次 TDD Red で追加 |
