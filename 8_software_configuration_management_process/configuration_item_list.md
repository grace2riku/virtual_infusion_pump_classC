...
| CI-SRC-006 | 永続化(UNIT-003.*)| `src/vip_persist/` | **UNIT-003.3 Atomic File Writer v0.1**(Step 19 B5、`atomic_writer.py`、78 stmt、カバレッジ 100%、MC/DC 100%、RCM-015 前提実装:temp→rename + 1 世代 bak)+ UNIT-003.1 / 003.2 骨格のみ(`__init__.py`、Step 19 B1)| C | 運用中(UNIT-003.3 完了、UNIT-003.1 / 003.2 は TDD 未着手) |
| CI-SRC-007 | 整合性検証(UNIT-004.*)| `src/vip_integrity/` | 骨格のみ(Step 19 B1)| C | 骨格完了、TDD 未着手 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.5 | ドラフト(セルフ承認、Step 19 B5:UNIT-003.3 実施結果を §9.2 / §11 に反映、Pass 21 tests / カバレッジ 100% / MC/DC 100%。§7.3.4 を SDD §4.4 に整合化(API 3 系統統一 / ロック撤回 / OSError 型統一 / SIGKILL 試験を ITPR 申し送り)、MC/DC 目標 95%→100% 引き上げ、MINOR 区分・CR 不要)|
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.12 | ドラフト(セルフ承認、Step 19 B5 反映:CI-SRC-006 に UNIT-003.3 v0.1 を追記、CI-TD-001f 新規登録、CI-DOC-UTPR v0.5 反映、自己参照 v0.12) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.14 | 継続更新(ステップ毎に追記、Step 19 B5 反映:UNIT-003.3 Atomic File Writer TDD 実装、カバレッジ 100% / MC/DC 100% 達成、UTPR §7.3.4 を SDD §4.4 に整合化(API 統一 / ロック撤回 / OSError 型統一 / SIGKILL 試験 ITPR 申し送り))|
...
| (予定) CI-TD-001c | UNIT-001.2 / 001.3 / 001.5 / 002.1 / 002.2 / 002.3 / 003.1 / 003.2 / 004.x / 005.x 試験 | `tests/unit/test_*.py` | Step 19 B6+ 以降で UNIT-004.1 などを順次 TDD Red で追加 |
