...
| CI-SRC-006 | 永続化(UNIT-003.*)| `src/vip_persist/` | **UNIT-003.3 Atomic File Writer v0.1**(Step 19 B5、`atomic_writer.py`、78 stmt、カバレッジ 100%、MC/DC 100%、RCM-015 前提実装:temp→rename + 1 世代 bak)+ **UNIT-003.1 Serializer v0.1**(Step 19 B7、`serializer.py`、47 stmt、カバレッジ 100%、MC/DC 100%、RCM-015 前提実装:Decimal/State/bytes 3 種タグ + sort_keys 決定論性 + build_persisted_record ファクトリ)+ **共通データモデル v0.2**(Step 19 B6 初版 + Step 19 B7 で `PersistedRecord` 追加、`records.py`、45 stmt、カバレッジ 100%、pydantic frozen モデル `Settings` / `RuntimeState` / `RawPersistedRecord` / `PersistedRecord` / `TrustedRecord`)+ UNIT-003.2 骨格のみ(`__init__.py`、Step 19 B1)| C | 運用中(UNIT-003.1 / 003.3 + 共通データモデル完了、UNIT-003.2 は TDD 未着手) |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.7 | ドラフト(セルフ承認、Step 19 B7:UNIT-003.1 実施結果を §9.2 / §11 に反映、Pass 26 tests / カバレッジ 100% / MC/DC 100%。§7.3.6 を新規詳細化(骨格 → 詳細テーブル UT-003.1-01〜17、7 論点 + 1 実装時論点を推奨方針で解消:別モデル型設計 / build_persisted_record ファクトリ / State 名前シリアライズ / records.py 不変 / hypothesis max_examples=200 / current_schema_version 関数 / MC/DC 100% 引き上げ / `__bytes__` タグ)、MINOR 区分・CR 不要、残骨格を 12 → 11 ユニットに更新)|
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.14 | ドラフト(セルフ承認、Step 19 B7 反映:CI-SRC-006 に UNIT-003.1 Serializer v0.1 追記 + 共通データモデル v0.2 昇格(`PersistedRecord` 追加)、CI-TD-001h 新規登録、CI-DOC-UTPR v0.7 反映、自己参照 v0.14) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.16 | 継続更新(ステップ毎に追記、Step 19 B7 反映:UNIT-003.1 Serializer TDD 実装、カバレッジ 100% / MC/DC 100% 達成、残 12 → 11 ユニット、UTPR §7.3.6 を新規詳細化(7 論点 + 1 実装時論点を推奨方針で解消))|
...
| (予定) CI-TD-001c | UNIT-001.2 / 001.3 / 001.5 / 002.1 / 002.2 / 002.3 / 003.2 / 004.2 / 005.x 試験 | `tests/unit/test_*.py` | Step 19 B8+ 以降で UNIT-003.2 Checksum Verifier などを順次 TDD Red で追加 |
